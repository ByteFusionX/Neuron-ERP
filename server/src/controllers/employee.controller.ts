...
export const getEmployees = async (
  ...
  try {
    const employees = await Employee.find(
      { isDeleted: { $ne: true }, isBlocked: { $ne: true } },
      { password: 0 },
    )
      .sort({ createdDate: -1 })
      .populate("department");
...
export const getPresaleManagers = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
...
export const getPresaleEngineers = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
...
export const getEmployeesForCustomerTransfer = async (
  ...
  try {
    ...
    const reportedEmployee = await employeeModel.findOne({
      _id: employeeAccess.reportingTo,
    });
    ...
    if (employeeAccess?.category?.privileges?.employee?.viewReport == "none") {
      res
        .status(200)
        .json(
          reportedEmployee && !reportedEmployee.isBlocked
            ? [reportedEmployee]
            : [],
        );
    }
    ...
    let allEmployees = await Employee.aggregate([
      {
        $match: { ...accessFilter, isBlocked: { $ne: true } },
      },
      {
        $lookup: {
          from: "categories",
    ...
    if (reportedEmployee && !reportedEmployee.isBlocked) {
    ...
    const employeesWithoutAccess = await Promise.all(
      allEmployees.map(async (employee) => {
        ...
        const isShared = customer.sharedWith.some(
          (sharedEmployeeId) =>
            String(sharedEmployeeId) === String(employee._id),
        );
        if (isShared) {
        ...
        switch (privilege) {
          ...
          case "reported":
            ...
            const reportedEmployees = await getAllReportedEmployees(
              customer.createdBy,
            );
            return {
              employee,
    ...
    return res
      .status(204)
      .json({ message: "All employees have access to the customer" });
  } catch (error) {
    console.log(error);
    next(error);
  }
...
export const isEmployeePresent = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  ...
  try {
    ...
    if (employeeExist) {
      ...
      if (employeeData[0]) {
        return res
          .status(200)
          .json({ access: true, employeeData: employeeData[0] });
      } else {
...
export const getPasswordForEmployee = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
...
export const updateTarget = async (
  ...
  try {
    ...
    const isDuplicate = employee.targets.some(
      (t) =>
        t._id.toString() === targetObjectId.toString() &&
        t.year === target.year,
    );
...
export const changePasswordOfEmployee = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
