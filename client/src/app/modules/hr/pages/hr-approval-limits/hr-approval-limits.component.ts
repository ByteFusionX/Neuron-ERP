...
import { WorkflowStepsDialogComponent } from '../workflow-steps-dialog/workflow-steps-dialog.component';
import { SkeltonLoadingComponent } from 'src/app/shared/components/skelton-loading/skelton-loading.component';

@Component({
  selector: 'app-hr-approval-limits',
  standalone: true,
  templateUrl: './hr-approval-limits.component.html',
  styleUrls: ['./hr-approval-limits.component.css'],
...
export class HrApprovalLimitsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.subscriptions.add(
      this._employeeService.employeeData$.pipe(filter((e) => !!e), take(1)).subscribe((employee) => {
        ...
        if (this.privileges?.portalManagement?.department || this.categorySection) {
          this.loadWorkflows();
        }
      })
    );
