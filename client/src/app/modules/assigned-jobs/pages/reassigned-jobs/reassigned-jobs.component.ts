...
@Component({
  ...
  imports: [NgIf, NgSwitch, NgSwitchCase, AsyncPipe, DataGridComponent, DetailOverviewComponent, DetailCommentsComponent, DetailDocumentsComponent, DetailTaskListComponent, ViewEstimationComponent, EstimationFormDrawerComponent]
})
export class ReassignedJobsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  get pageTitle(): string {
    return this.viewAssignedFor ? 'Reassigned Jobs' : 'Assigned Jobs';
  }
  ...
  private buildColumns(): void {
    this.columns = [
      { key: 'enqId', label: 'Enquiry ID', valueGetter: (r) => r.enquiryId, cellClass: (r) => r.reAssignedSeen ? 'text-violet-500' : 'text-orange-500' },
      ...
      { key: 'assignedBy', label: 'Assign. By', valueGetter: (r) => `${r.preSale?.presalePerson?.[0]?.firstName ?? ''} ${r.preSale?.presalePerson?.[0]?.lastName ?? ''}`.trim() },
      { key: 'assignedFor', label: 'Assign. For', visible: this.viewAssignedFor, valueGetter: (r) => r.reAssigned?.[0] ? `${r.reAssigned[0].firstName} ${r.reAssigned[0].lastName}` : '—' },
  ...
  getJobsData() {
    ...
    if (this.userId) {
      ...
      this.subscriptions.add(
        this._enquiryService.getPresale(this.page, this.row, 'reassigned', access, this.userId, this.searchQuery).subscribe({
          next: (data) => {
  ...
  onRowOpen(row: any): void {
    ...
    if (row?._id && !row.reAssignedSeen) {
      this.markJobAsViewed(row._id);
      row.reAssignedSeen = true;
    }
  }

  markJobAsViewed(jobId: string) {
    this._enquiryService.markReassignJobAsViewed(jobId).subscribe();
  }
    ...
    return [
      {
        ...
        fields: [
          ...
          { type: 'field', label: 'Assigned By', value: `${row.preSale?.presalePerson?.[0]?.firstName ?? ''} ${row.preSale?.presalePerson?.[0]?.lastName ?? ''}`.trim() },
          { type: 'field', label: 'Department', value: row.department?.[0]?.departmentName },
          { type: 'field', label: 'Assigned For', value: row.reAssigned?.[0] ? `${row.reAssigned[0].firstName} ${row.reAssigned[0].lastName}` : '—', visible: this.viewAssignedFor },
        ],
      },
    ];
  }
  ...
  commentsFor(row: any): DetailComment[] {
    ...
    const assignedByName = row.preSale?.presalePerson?.[0] ? `${row.preSale.presalePerson[0].firstName} ${row.preSale.presalePerson[0].lastName}` : 'Presale Person';
  ...
  // ---- Row actions: feedback / reject / send ---------------------------------------------
  ...
  async onSendClicked(row: any): Promise<void> {
    ...
    const { confirmed } = await this.confirm.open({
      tone: 'approve',
      title: 'Send job back?',
      message: 'This action is irreversible and send the assigned task back to the salesperson. You can also verify by other employees. Please ensure all files are selected before proceeding.',
      confirmLabel: 'Send',
      cancelLabel: 'Cancel',
    });
    if (!confirmed) return;
  ...
  async onRejectJob(row: any): Promise<void> {
    ...
    this._enquiryService.rejectJob(row._id, reason ?? '', 'Engineer').subscribe({
      next: (res) => {
        if (res.success) {
          this.rows = this.rows.filter((r) => r._id !== row._id);
          this.total = this.rows.length;
