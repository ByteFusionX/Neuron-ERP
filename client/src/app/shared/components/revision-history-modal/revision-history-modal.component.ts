...
export interface RevisionHistoryModalData {
  /** Fetches the revisions, newest first. Any entity whose API returns the quote revision shape can use this modal. */
  load: () => Observable<{ revisions: QuoteRevision[] }>;
  /** Shown in the subtitle and the "original" labels, e.g. "quotation". Defaults to "record". */
  entityLabel?: string;
  /** Reference of the record being viewed, e.g. the quote number. */
  ref?: string;
}
...
@Component({
  ...
  template: `
    <app-modal-layout
      ...
      [subtitle]="data.ref ? 'Commercial content of ' + data.ref + ' at each revision' : 'Commercial content at each revision'"
          ...
          <ol class="flex gap-2 overflow-x-auto md:flex-col md:overflow-visible">
            <li *ngFor="let r of rows" class="shrink-0 md:shrink">
              ...
              >
                ...
                <p class="mt-1 truncate text-xs text-gray-600 dark:text-gray-400">{{ r.by || 'Original ' + entityLabel }}</p>
          ...
          <div *ngIf="selected as s" class="min-w-0 space-y-4">
            <div class="rounded-lg border border-gray-200 p-3 dark:border-erp-border-dark">
              ...
              <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                ...
                <ng-template #original>The {{ entityLabel }} as it was first created.</ng-template>
              </p>
...
export class RevisionHistoryModalComponent {
  ...
  get entityLabel(): string {
    return this.data.entityLabel || 'record';
  }
  ...
  constructor(
    ...
    this.data.load().subscribe({
      next: (res) => {
        ...
        this.loading = false;
        if (!this.rows.length) this.error = `No revisions recorded for this ${this.entityLabel}.`;
      },
      error: () => {
        this.loading = false;
